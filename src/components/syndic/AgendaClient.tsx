...
const CUSTOM_COLORS: Record<string, string> = {
  ...
  autre: 'bg-coplio-green-light text-coplio-green',
}
    ...
    <div className="space-y-6 animate-fade-in">
              ...
              <div className="flex flex-wrap gap-2">
                {TYPES_CUSTOM.map(t => (
                  <button key={t.value} type="button" onClick={() => setForm(f => ({ ...f, type: t.value }))}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium border-2 transition-all ${form.type === t.value ? 'border-coplio-green bg-coplio-green-light text-coplio-green' : 'border-border text-muted-foreground hover:border-coplio-green/30'}`}>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          { label: 'Événement', color: 'bg-coplio-green-light text-coplio-green' },
        ].map(l => (
            ...
            <div key={month}>
              ...
              <div className="space-y-2">
                {items.map(({ id, date, isCustom, data }) => {
                      ...
                      <Link key={`sys-${id}`} href={ev.lien}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            ...
                            {isToday && <span className="text-xs bg-coplio-green text-white px-1.5 py-0.5 rounded-full font-medium flex-shrink-0">Aujourd&apos;hui</span>}
                          </div>
                  ...
                  const color = CUSTOM_COLORS[ev.type] ?? 'bg-coplio-green-light text-coplio-green'
