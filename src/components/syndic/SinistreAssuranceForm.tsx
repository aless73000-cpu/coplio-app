  ...
  return (
    <div className="coplio-card">
      <h2 className="font-semibold text-coplio-text mb-4 flex items-center gap-2">
        <Shield className="w-4 h-4 text-[#374151]" />Suivi assurance
      </h2>
          ...
          <div className="p-3 bg-coplio-bg rounded-xl">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 h-2 bg-border rounded-full overflow-hidden">
                <div className={`h-full rounded-full transition-all ${taux >= 80 ? 'bg-[#374151]' : taux >= 50 ? 'bg-amber-400' : 'bg-red-500'}`}
                  style={{ width: `${Math.min(100, taux)}%` }} />
              </div>
              <span className={`text-sm font-bold ${taux >= 80 ? 'text-[#374151]' : taux >= 50 ? 'text-amber-600' : 'text-red-600'}`}>
                {taux}%
              </span>
            </div>
          </div>
