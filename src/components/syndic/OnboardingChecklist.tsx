...
export function OnboardingChecklist({ steps }: Props) {
  ...
  return (
    <div className="coplio-card border-[#111827]/30 bg-gradient-to-br from-white to-slate-100/30 mb-0">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3 flex-1">
          <div className="w-8 h-8 bg-[#111827]/10 rounded-lg flex items-center justify-center flex-shrink-0">
            <Sparkles className="w-4 h-4 text-[#111827]" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 className="font-semibold text-coplio-text text-sm">Démarrez avec Coplio</h2>
              <span className="text-xs font-medium text-[#111827] bg-slate-100 px-2 py-0.5 rounded-full">
              ...
              <div
                ...
                className={`flex items-start gap-3 p-2.5 rounded-lg transition-colors ${
                  step.done
                    ? 'opacity-50'
                    : isNext
                    ? 'bg-white border border-[#111827]/20 shadow-sm'
                    : 'hover:bg-white/60'
                }`}
              >
                {step.done ? (
                  <CheckCircle2 className="w-5 h-5 text-[#111827] flex-shrink-0 mt-0.5" />
                ) : (
                  <Circle className={`w-5 h-5 flex-shrink-0 mt-0.5 ${isNext ? 'text-[#111827]' : 'text-border'}`} />
                )}
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-medium ${step.done ? 'line-through text-muted-foreground' : 'text-coplio-text'}`}>
                    {step.label}
                    {isNext && (
                      <span className="ml-2 text-[10px] font-semibold text-[#111827] bg-slate-100 px-1.5 py-0.5 rounded-full">
                        À faire
                      </span>
                    )}
                  </p>
                ...
                {!step.done && (
                  <Link
                    ...
                    className={`text-xs font-semibold flex-shrink-0 mt-0.5 transition-colors ${
                      isNext
                        ? 'text-white bg-[#111827] px-2.5 py-1 rounded-lg hover:bg-[#111827]/90'
