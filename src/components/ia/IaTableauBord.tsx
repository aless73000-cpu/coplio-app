  ...
  if (!analyseData) {
    return (
      <div className="coplio-card text-center py-12">
        ...
        <button
          onClick={loadAnalyse}
          className="bg-[#374151] text-white px-5 py-2.5 rounded-lg text-sm font-medium hover:bg-[#374151]/90 transition-colors"
        >
          Lancer l&apos;analyse
        </button>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      {analyseData.analyse.resume && (
        <div className="coplio-card bg-slate-100 border-[#374151]/20">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="coplio-card">
          <h2 className="font-semibold text-coplio-text mb-4 flex items-center gap-2">
            <TrendingDown className="w-4 h-4 text-[#374151]" />Score de risque par copropriété
          </h2>
          ...
          <button
            ...
            className="mt-4 text-xs text-[#374151] hover:underline"
          >
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-2">
                {analyseData.analyse.anomalies.map((a, i) => (
                  <div key={i} className={`p-3 rounded-xl border text-sm ${
                    ...
                    {a.copropriete && (
                      <span className="text-xs font-medium text-[#374151] mt-1 block">{a.copropriete}</span>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div className="coplio-card">
            <h2 className="font-semibold text-coplio-text mb-3 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-[#374151]" />Recommandations
            </h2>
            <ul className="space-y-2">
              {(analyseData.analyse.recommandations ?? []).map((r, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-coplio-text">
                  <span className="text-[#374151] font-bold flex-shrink-0 mt-0.5">→</span>{r}
                </li>
              ))}
            </ul>
          </div>
          ...
          {(analyseData.analyse.points_positifs ?? []).length > 0 && (
            <div className="coplio-card">
              <h2 className="font-semibold text-coplio-text mb-3 flex items-center gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#374151]" />Points positifs
              </h2>
              <ul className="space-y-2">
                {analyseData.analyse.points_positifs.map((p, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-coplio-text">
                    <span className="text-[#374151] flex-shrink-0 mt-0.5">✓</span>{p}
                  </li>
                ))}
              </ul>
            </div>
