    ...
    return (
      <div className="coplio-card text-center py-16">
        <div className="w-14 h-14 bg-coplio-green-light rounded-full flex items-center justify-center mx-auto mb-4">
          <Vote className="w-7 h-7 text-coplio-green" />
        </div>
        ...
        return (
          <div key={vote.id} className="coplio-card">
            <div className="flex items-start gap-3 mb-4">
              ...
              {hasVoted && (
                <div className="flex items-center gap-1 text-coplio-green text-xs font-medium flex-shrink-0">
                  ...
                  <div key={option.id}>
                    {canVote ? (
                      <button
                        ...
                        className="w-full text-left px-4 py-3 rounded-xl border-2 border-border hover:border-coplio-green hover:bg-coplio-green-light transition-all text-sm font-medium text-coplio-text flex items-center justify-between group disabled:opacity-60"
                      >
                        <span>{option.label}</span>
                        {voting === option.id
                          ? <Loader2 className="w-4 h-4 animate-spin text-coplio-green" />
                          : <span className="text-xs text-muted-foreground group-hover:text-coplio-green">Voter →</span>
                        }
                      </button>
                    ) : (
                      <div className={`px-4 py-3 rounded-xl border-2 transition-all ${
                        isMyChoice ? 'border-coplio-green bg-coplio-green-light' : 'border-border bg-coplio-bg'
                      }`}>
                        <div className="flex justify-between text-sm mb-1.5">
                          <span className={`font-medium ${isMyChoice ? 'text-coplio-green' : 'text-coplio-text'}`}>
                            {option.label}
                            {isMyChoice && ' ✓'}
                          </span>
                        ...
                        <div className="h-1.5 bg-white/60 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all duration-500 ${isMyChoice ? 'bg-coplio-green' : 'bg-muted-foreground/30'}`}
                            style={{ width: `${pct}%` }}
                          />
                        </div>
                      </div>
