      ...
      <div className="px-5 py-5 border-b border-border">
        <div className="flex items-center justify-between">
          <Link href="/accueil" className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-[#374151] rounded-xl flex items-center justify-center">
        ...
        {NAV_GROUPS.map(({ href, label, sub, icon: Icon, matches }) => {
          ...
          return (
            ...
            >
              ...
              {active && (
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-5 bg-[#374151] rounded-r-full" />
              )}
              <Icon className={cn(
                'w-4 h-4 flex-shrink-0 transition-colors',
                active ? 'text-[#374151]' : 'text-muted-foreground group-hover:text-coplio-text'
              )} />
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  <span className={cn(
                    'text-sm font-medium leading-tight',
                    active ? 'text-[#374151]' : 'text-coplio-text'
                  )}>
                    {label}
                  </span>
                ...
                {sub && (
                  <p className={cn(
                    ...
                    active ? 'text-[#374151]/70' : 'text-muted-foreground'
                  )}>
                    {sub}
                  </p>
                )}
      ...
      <div className="px-3 pb-4 pt-1">
        ...
        >
          <div className={cn(
            ...
            isAccountActive ? 'bg-[#374151] text-white' : 'bg-coplio-bg text-coplio-text'
          )}>
            {initials}
          </div>
          <div className="flex-1 min-w-0">
            <p className={cn(
              ...
              isAccountActive ? 'text-[#374151]' : 'text-coplio-text'
            )}>
              {prenom} {nom}
            </p>
