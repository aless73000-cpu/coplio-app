    ...
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <div className="max-w-7xl mx-auto px-5 h-[66px] flex items-center justify-between">
        ...
        <Link href="/" className="flex items-center gap-2.5 flex-shrink-0">
          <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${scrolled ? 'bg-[#111827]' : 'bg-white/10 border border-white/15'}`}>
            <span className={`text-xs font-bold ${scrolled ? 'text-white' : 'text-white/70'}`}>C</span>
          </div>
          <span className={`font-bold text-xl ${scrolled ? 'text-[#111827]' : 'text-white/80'}`} style={{ letterSpacing: '-0.02em' }}>
            Coplio
          </span>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-0.5">
          ...
          >
            <button className={`flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-medium transition-all ${
              scrolled ? 'text-slate-500 hover:text-[#111827] hover:bg-slate-50' : 'text-white/50 hover:text-white/80 hover:bg-white/5'
            }`}>
              Fonctionnalités
            ...
            <div className={`absolute top-full left-1/2 -translate-x-1/2 mt-3 w-[560px] bg-white rounded-2xl shadow-2xl shadow-black/10 border border-slate-100 p-5 transition-all duration-200 origin-top ${
              ...
              <div className="grid grid-cols-2 gap-1 mb-4">
                {modules.map(({ icon: Icon, label, desc }, idx) => (
                  <button key={label} onClick={() => handleModuleClick(idx)}
                    ...
                    <div className="w-8 h-8 bg-slate-100 rounded-lg flex items-center justify-center flex-shrink-0 group-hover:bg-[#111827] transition-colors">
                      <Icon className="w-4 h-4 text-slate-500 group-hover:text-white transition-colors" />
                    </div>
                    <div>
                      <div className="text-sm font-semibold text-[#111827] leading-tight">{label}</div>
              ...
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                ...
                <Link href="/register" className="flex items-center gap-1 text-xs font-bold text-[#111827] hover:underline">
        ...
        <div className="hidden md:flex items-center gap-3">
          <Link href="/portail" className={`text-sm transition-colors ${scrolled ? 'text-slate-400 hover:text-[#111827]' : 'text-white/40 hover:text-white/70'}`}>
            Espace copropriétaire
          </Link>
          <Link href="/login" className={`text-sm transition-colors ${scrolled ? 'text-slate-400 hover:text-[#111827]' : 'text-white/40 hover:text-white/70'}`}>
            Se connecter
          </Link>
          <Link
            href="/register"
        ...
        >
          {mobileOpen
            ? <X className={`w-5 h-5 ${scrolled ? 'text-[#111827]' : 'text-white/70'}`} />
            : <Menu className={`w-5 h-5 ${scrolled ? 'text-[#111827]' : 'text-white/70'}`} />
          }
        </button>
      </div>
      ...
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-slate-100 px-5 py-5">
          ...
          <div className="space-y-0.5 border-t border-slate-100 pt-4 mb-4">
            <a href="#tarifs"     onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 text-sm text-slate-500 hover:text-[#111827] rounded-xl hover:bg-slate-50">Tarifs</a>
            <a href="#faq"        onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 text-sm text-slate-500 hover:text-[#111827] rounded-xl hover:bg-slate-50">FAQ</a>
            <Link href="/portail" onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 text-sm text-slate-500 hover:text-[#111827] rounded-xl hover:bg-slate-50">Espace copropriétaire</Link>
            <Link href="/login"   onClick={() => setMobileOpen(false)} className="block px-3 py-2.5 text-sm text-slate-500 hover:text-[#111827] rounded-xl hover:bg-slate-50">Se connecter</Link>
          </div>
          <Link href="/register" onClick={() => setMobileOpen(false)}
            className="flex items-center justify-center gap-2 bg-[#111827] text-white px-4 py-3.5 rounded-xl font-semibold text-sm hover:bg-[#1F2937] transition-colors">
