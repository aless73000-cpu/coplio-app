  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 py-4">
      ...
      {total_du > 0 ? (
        ...
        <div className="bg-slate-100 border border-[#374151]/20 rounded-2xl p-5 flex items-center gap-4">
          <div className="w-12 h-12 bg-[#374151] rounded-xl flex items-center justify-center flex-shrink-0">
            <CheckCircle2 className="w-6 h-6 text-white" />
          </div>
          <div>
            <p className="font-semibold text-[#374151] text-lg">Votre compte est à jour</p>
            <p className="text-sm text-[#374151]/70 mt-0.5">Aucun paiement en attente · Continuez comme ça !</p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 divide-y sm:divide-y-0 sm:divide-x divide-border">
          <div className="px-4 py-4">
            ...
            <p className={`text-lg font-bold ${soldeCompte >= 0 ? 'text-[#374151]' : 'text-coplio-red'}`}>
          ...
          <div className="px-4 py-4">
            ...
            <p className="text-base font-bold text-[#374151]">{formatEuro(totalPaye)}</p>
            ...
            {years.map((year) => (
              <div key={year}>
                ...
                <div className="px-5 py-2.5 bg-coplio-bg flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${year === currentYear ? 'bg-[#374151]' : 'bg-gray-300'}`} />
                  <span className={`text-xs font-semibold uppercase tracking-wide ${year === currentYear ? 'text-[#374151]' : 'text-muted-foreground'}`}>
                    {year}
                  </span>
                </div>
