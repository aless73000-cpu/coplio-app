      ...
      <div className="grid gap-5">
        ...
        <div className="coplio-card space-y-2">
          ...
          <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
            ...
            <dd className={cabinet.addon_portail_actif ? 'text-coplio-green font-medium' : 'text-muted-foreground'}>
        ...
        <div className="coplio-card">
            ...
            {PLAN_OPTIONS.map(plan => (
              <label
                ...
                className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition-colors ${
                  selectedPlan === plan ? 'border-coplio-green bg-coplio-green/5' : 'border-border hover:bg-coplio-bg'
                }`}
              >
                <div className="flex items-center gap-2">
                  <input type="radio" name="plan" value={plan} checked={selectedPlan === plan} onChange={() => setSelectedPlan(plan)} className="accent-coplio-green" />
          ...
          <button
            ...
            className="w-full bg-coplio-green text-white font-medium py-2.5 rounded-lg hover:bg-coplio-green/90 transition-colors disabled:opacity-60 flex items-center justify-center gap-2 text-sm"
          >
            ...
            <div className="space-y-2">
              {profiles.map(p => (
                <div key={p.id} className="flex items-center gap-3 p-2 bg-coplio-bg rounded-lg">
                  <div className="w-8 h-8 bg-coplio-green/10 rounded-full flex items-center justify-center flex-shrink-0">
          ...
          <div className="flex gap-3">
            {isSuspended ? (
              <button
                onClick={reactiver}
                disabled={saving}
                className="flex-1 flex items-center justify-center gap-2 bg-coplio-green text-white font-medium py-2.5 rounded-lg hover:bg-coplio-green/90 transition-colors disabled:opacity-60 text-sm"
              >
