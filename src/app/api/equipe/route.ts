...
export async function GET() {
  try {
    ...
    captureException(err, { context: 'equipe-get' })
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (inviteError) {
      ...
      captureException(inviteError, { context: 'equipe-post-invite' })
    ...
    captureException(err, { context: 'equipe-post' })
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 })
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    captureException(err, { context: 'equipe-delete' })
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 })
  }
}
