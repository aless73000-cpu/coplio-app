...
export const POST = withErrorHandler(async (request: Request) => {
  ...
  // 3. Email de bienvenue avec lien de confirmation (non bloquant, 3 tentatives)
  //    Le CTA de l'email pointe vers confirmUrl pour valider l'adresse email.
  fireAndForget(
    () => Email.welcomeSyndic({ prenom, nomCabinet, confirmUrl }, email),
    { attempts: 3, onFailure: err => captureException(err, { context: 'register welcome email' }) }
  )
