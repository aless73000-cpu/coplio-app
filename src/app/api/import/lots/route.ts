...
export async function POST(request: NextRequest) {
  ...
  const limit = await rateLimit(`import-lots:${ip}`, { max: 10, windowMs: 60 * 60 * 1000 })
  ...
  try {
    ...
    // Vérification du quota de lots avant import
    const quota = await checkQuota(profile.cabinet_id, 'lots', rows.length)
    ...
    captureException(err, { context: 'import-lots' })
