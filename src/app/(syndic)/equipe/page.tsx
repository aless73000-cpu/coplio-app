    ...
    <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3">
            <select value={selectedCopro} onChange={e => setSelectedCopro(e.target.value)}
              className="flex-1 px-3 py-2.5 text-sm border border-border rounded-xl bg-white focus:outline-none focus:ring-2 focus:ring-[#374151]/20">
              {coproprietes.map(c => <option key={c.id} value={c.id}>{c.nom}</option>)}
            </select>
            <button onClick={() => setShowConseilForm(v => !v)}
              className="flex items-center gap-2 px-4 py-2.5 bg-[#374151] text-white text-sm font-medium rounded-xl hover:bg-[#374151]/90 transition-colors">
              ...
              <form onSubmit={handleAddConseil} className="space-y-3">
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={savingConseil}
                    className="flex items-center gap-2 bg-[#374151] text-white text-sm px-4 py-2 rounded-lg hover:bg-[#374151]/90 disabled:opacity-60">
            ...
            {membres.map((m) => (
              <div key={m.id} className="flex items-center gap-4 px-6 py-4">
                <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center flex-shrink-0">
                  <span className="text-[#374151] text-sm font-bold">{initials(m)}</span>
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
      ...
      <div className="coplio-card space-y-4">
        ...
        <form onSubmit={handleInvite} className="flex gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 bg-[#374151] text-white text-sm font-medium rounded-xl
      ...
      <div className="flex items-center gap-3 p-4 bg-coplio-bg rounded-xl border border-border text-sm">
        ...
        <p className="text-muted-foreground">
          ...
          <Link href="/facturation" className="text-[#374151] font-medium hover:underline">
            Voir les limites
          </Link>
        </p>
      </div>
