  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in">
      ...
      {searchParams.success && (
        <div className="p-4 bg-slate-100 border border-[#374151]/20 rounded-xl">
          <p className="text-[#374151] font-medium">✓ Abonnement activé avec succès !</p>
          <p className="text-sm text-[#374151]/80 mt-1">Vous avez maintenant accès à toutes les fonctionnalités de votre plan.</p>
        </div>
      )}
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                  isActive ? 'bg-slate-100 text-[#374151]' : 'bg-coplio-amber-bg text-coplio-amber'
                }`}>
            ...
            return (
              <div
                ...
                className={`relative rounded-2xl border p-6 ${
                  isPro
                    ? 'border-[#374151] shadow-md bg-slate-100/30'
                    : 'border-border bg-white'
                }`}
              >
                {isPro && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="bg-[#374151] text-white text-xs font-semibold px-3 py-1 rounded-full">
                      Le plus populaire
                    </span>
                  </div>
                )}
                ...
                <ul className="space-y-2 mb-6">
                  {plan.features.map((feature) => (
                    <li key={feature} className="flex items-start gap-2 text-sm text-coplio-text">
                      <Check className="w-4 h-4 text-[#374151] flex-shrink-0 mt-0.5" />
                      {feature}
                    </li>
                  ))}
                </ul>
                ...
                {isCurrentPlan ? (
                  <div className="w-full py-2.5 bg-slate-100 text-[#374151] text-center rounded-xl text-sm font-medium">
                    Plan actuel
                  </div>
                ) : (
