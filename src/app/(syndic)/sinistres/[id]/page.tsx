              ...
              <div key={step.key} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  <div className={`w-9 h-9 rounded-full flex items-center justify-center border-2 transition-colors ${
                    isDone ? 'bg-coplio-green border-coplio-green text-white' :
                    isCurrent ? 'bg-white border-coplio-green text-coplio-green' :
                    'bg-white border-border text-muted-foreground'
                  }`}>
                    {isDone ? <CheckCircle2 className="w-4 h-4" /> : <Icon className="w-4 h-4" />}
                  </div>
                  <span className={`text-xs mt-1 text-center leading-tight max-w-[60px] ${
                    isCurrent ? 'text-coplio-green font-medium' : isDone ? 'text-coplio-text' : 'text-muted-foreground'
                  }`}>
                    {step.label}
                  </span>
                </div>
                {i < STATUS_STEPS.length - 1 && (
                  <div className={`flex-1 h-0.5 mb-4 mx-1 ${i < currentStepIndex ? 'bg-coplio-green' : 'bg-border'}`} />
                )}
              </div>
            )
          })}
            ...
            <div className="flex flex-wrap gap-2">
              {STATUS_STEPS.filter(s => s.key !== sinistre.status).map(step => (
                <form key={step.key} action={`/api/sinistres/${sinistre.id}/status`} method="POST">
                  ...
                  <button type="submit" className="text-xs px-3 py-1.5 border border-border rounded-lg hover:border-coplio-green hover:text-coplio-green transition-colors">
                    → {step.label}
                  </button>
                </form>
              ))}
            ...
            <dl className="space-y-2.5 text-sm">
              <div>
                ...
                <dd className="font-medium text-coplio-text mt-0.5">
                  <Link href={`/coproprietes/${sinistre.copropriete?.id}`} className="text-coplio-green hover:underline">
