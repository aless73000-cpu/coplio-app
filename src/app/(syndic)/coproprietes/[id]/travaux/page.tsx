...
const STATUTS = [
  ...
  { value: 'realisation', label: 'En cours', icon: Hammer, color: 'bg-coplio-green-light text-coplio-green' },
          ...
          return (
            <button key={s.value} onClick={() => setFilterStatut(filterStatut === s.value ? '' : s.value)}
              className={`flex-1 min-w-20 text-center py-2 px-2 rounded-xl text-xs font-medium transition-all border-2 ${
                filterStatut === s.value ? 'border-coplio-green' : 'border-transparent'
              } ${s.color}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input value={form.titre} onChange={e => setForm(f => ({ ...f, titre: e.target.value }))} required
                  placeholder="Ex: Ravalement façade nord" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-coplio-green" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" value={form.montant_estime} onChange={e => setForm(f => ({ ...f, montant_estime: e.target.value }))}
                  placeholder="0" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-coplio-green" />
              </div>
              <div className="col-span-2">
                  ...
                  <div className="flex items-center gap-1 flex-shrink-0">
                    {nextStatut && (
                      <button onClick={() => handleStatut(t.id, nextStatut)}
                        className="text-xs text-coplio-green hover:bg-coplio-green-light px-2 py-1 rounded-lg transition-colors font-medium whitespace-nowrap">
                      ...
                      <div className="space-y-2">
                        ...
                        {t.etapes.sort((a, b) => new Date(a.created_at).getTime() - new Date(b.created_at).getTime()).map(e => (
                          <div key={e.id} className="flex gap-3 text-sm">
                            <div className="w-2 h-2 bg-coplio-green rounded-full mt-1.5 flex-shrink-0" />
                            <div>
                    ...
                    <div className="bg-coplio-bg rounded-xl p-3 space-y-2">
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <select
                          ...
                          className="col-span-1 px-2 py-1.5 text-xs border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-coplio-green bg-white">
                        ...
                        <input
                          ...
                          placeholder="Description..." className="col-span-1 px-2 py-1.5 text-xs border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-coplio-green" />
                        <div className="flex gap-1">
                          <input type="number"
                            value={etapeForm[t.id]?.montant ?? ''}
                            onChange={e => setEtapeForm(prev => ({ ...prev, [t.id]: { ...prev[t.id], montant: e.target.value } }))}
                            placeholder="Montant €" className="flex-1 px-2 py-1.5 text-xs border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-coplio-green min-w-0" />
                          <button onClick={() => handleAddEtape(t.id)}
                            className="px-2 py-1.5 bg-coplio-green text-white text-xs rounded-lg hover:bg-coplio-green/90 flex-shrink-0">
