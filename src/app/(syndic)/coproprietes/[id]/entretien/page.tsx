...
const STATUTS = [
  ...
  { value: 'realise', label: 'Réalisé', color: 'text-coplio-green bg-coplio-green-light' },
    ...
    <div className="max-w-3xl space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="coplio-card text-center">
          ...
          <p className="text-xl font-bold text-coplio-green">{entretiens.filter(e => e.statut === 'realise').length}</p>
        </div>
        <div className="coplio-card text-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="col-span-2">
                ...
                <input value={form.titre} onChange={e => setForm(f => ({ ...f, titre: e.target.value }))} required
                  placeholder="Ex: Révision chaudière" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-coplio-green" />
              </div>
              <div>
              ...
              <div className="col-span-2">
                ...
                <textarea value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} rows={2}
                  className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-coplio-green resize-none" placeholder="Détails de l'intervention..." />
              </div>
            </div>
      ...
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setTypeFilter('')}
          className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${!typeFilter ? 'bg-coplio-green text-white' : 'bg-coplio-bg text-muted-foreground hover:text-coplio-text'}`}>
        ...
        {TYPES.map(t => {
          ...
          return (
            <button key={t.value} onClick={() => setTypeFilter(typeFilter === t.value ? '' : t.value)}
              className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${typeFilter === t.value ? 'bg-coplio-green text-white' : 'bg-coplio-bg text-muted-foreground hover:text-coplio-text'}`}>
              {t.label} ({count})
            </button>
          )
        })}
                ...
                {isExpanded && e.description && (
                  <div className="mt-3 pt-3 border-t border-border">
                    ...
                    {e.prestataire?.telephone && (
                      <a href={`tel:${e.prestataire.telephone}`} className="text-sm text-coplio-green hover:underline mt-1 inline-block">
                        📞 {e.prestataire.telephone}
                      </a>
                    )}
                  </div>
