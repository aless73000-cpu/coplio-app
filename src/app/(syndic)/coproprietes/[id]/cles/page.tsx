    ...
    <div className="max-w-3xl space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="coplio-card text-center">
          ...
          <p className="text-xl font-bold text-[#374151]">{retournes}</p>
        </div>
      </div>
              ...
              <div className="flex flex-wrap gap-2">
                {TYPES.map(t => (
                  <button key={t.value} type="button" onClick={() => setForm(f => ({ ...f, type: t.value }))}
                    className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-sm font-medium border-2 transition-all ${form.type === t.value ? 'border-[#374151] bg-slate-100 text-[#374151]' : 'border-border text-muted-foreground'}`}>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} required
                  placeholder="Ex: Clé cave n°3" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#374151]/20" />
              </div>
              <div>
                ...
                <input value={form.localisation} onChange={e => setForm(f => ({ ...f, localisation: e.target.value }))}
                  placeholder="Ex: Parking, local vélos..." className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#374151]/20" />
              </div>
              <div>
                ...
                <input value={form.detenteur_nom} onChange={e => setForm(f => ({ ...f, detenteur_nom: e.target.value }))}
                  placeholder="Nom du détenteur" className="w-full px-3 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#374151]/20" />
              </div>
              <div>
      ...
      <div className="flex gap-2">
        {[{ label: 'Tous', val: null }, { label: 'En circulation', val: false }, { label: 'Retournés', val: true }].map(f => (
          <button key={String(f.val)} onClick={() => setFilterRetourne(f.val)}
            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${filterRetourne === f.val ? 'bg-[#374151] text-white' : 'bg-coplio-bg text-muted-foreground hover:text-coplio-text'}`}>
            {f.label}
          </button>
        ))}
      </div>
            ...
            return (
              <div key={item.id} className={`coplio-card flex items-center gap-3 ${item.retourne ? 'opacity-60' : ''}`}>
                <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${item.retourne ? 'bg-coplio-bg' : 'bg-slate-100'}`}>
                  <TypeIcon className={`w-4 h-4 ${item.retourne ? 'text-muted-foreground' : 'text-[#374151]'}`} />
                </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  <button onClick={() => toggleRetourne(item.id, item.retourne)}
                    className={`flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-lg transition-colors ${
                      item.retourne ? 'bg-coplio-bg text-muted-foreground hover:bg-slate-100 hover:text-[#374151]' : 'bg-slate-100 text-[#374151] hover:bg-[#374151] hover:text-white'
                    }`}>
