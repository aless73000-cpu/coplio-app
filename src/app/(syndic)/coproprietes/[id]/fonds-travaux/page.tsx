      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-coplio-text flex items-center gap-2">
            <PiggyBank className="w-6 h-6 text-coplio-green" />Fonds de travaux ALUR
          </h1>
        ...
        <>
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="coplio-card text-center">
              ...
              <p className="text-xl font-bold text-coplio-green">{fmt(current.solde_actuel)}</p>
            </div>
            <div className="coplio-card text-center">
          ...
          {current.objectif_5ans > 0 && (
            <div className="coplio-card">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm font-bold text-coplio-green">{progressPct}%</span>
              </div>
              <div className="h-3 bg-coplio-bg rounded-full overflow-hidden">
                <div className="h-full bg-coplio-green rounded-full transition-all" style={{ width: `${progressPct}%` }} />
              </div>
          ...
          <div className="coplio-card">
            <h2 className="font-semibold text-coplio-text mb-4 flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-coplio-green" />Paramètres
            </h2>
            ...
            <button onClick={handleSave} disabled={saving}
              className="mt-4 flex items-center gap-2 bg-coplio-green text-white text-sm font-medium px-4 py-2 rounded-lg hover:bg-coplio-green/90 transition-colors disabled:opacity-60">
          ...
          <div className="coplio-card">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setShowMvt(v => !v)}
                className="flex items-center gap-1.5 text-sm text-coplio-green font-medium hover:text-coplio-green/80">
              ...
              <form onSubmit={handleAddMvt} className="mb-4 p-4 bg-coplio-bg rounded-xl">
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <select value={mvtForm.type} onChange={e => setMvtForm(f => ({ ...f, type: e.target.value }))}
                      className="w-full px-2.5 py-2 text-sm border border-border rounded-lg bg-white focus:outline-none focus:ring-1 focus:ring-coplio-green">
                  ...
                  <div>
                    ...
                    <input type="number" value={mvtForm.montant} onChange={e => setMvtForm(f => ({ ...f, montant: e.target.value }))} required
                      className="w-full px-2.5 py-2 text-sm border border-border rounded-lg focus:outline-none focus:ring-1 focus:ring-coplio-green" />
                  </div>
                  <div>
                ...
                <div className="flex gap-2">
                  <button type="submit" disabled={savingMvt}
                    className="flex items-center gap-1.5 bg-coplio-green text-white text-sm px-3 py-1.5 rounded-lg hover:bg-coplio-green/90 disabled:opacity-60">
              ...
              <div className="space-y-2">
                {[...current.mouvements].sort((a, b) => new Date(b.date_mouvement).getTime() - new Date(a.date_mouvement).getTime()).map(m => (
                  <div key={m.id} className="flex items-center gap-3 p-3 rounded-lg hover:bg-coplio-bg transition-colors">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${m.type === 'retrait' ? 'bg-red-50 text-red-600' : 'bg-coplio-green-light text-coplio-green'}`}>
                    ...
                    <p className={`text-sm font-bold flex-shrink-0 ${m.type === 'retrait' ? 'text-coplio-red' : 'text-coplio-green'}`}>
