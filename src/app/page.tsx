...
/* ─────────────────────────────────────────────
   FEATURES SHOWCASE — onglets interactifs
───────────────────────────────────────────── */
const FEATURES_DATA = [
  {
    icon: BarChart3,
    ...
    preview: () => (
      <div className="space-y-3">
        <div className="grid grid-cols-2 gap-2">
          {[{v:'12',l:'Copropriétés',c:'text-[#0F6E56]',bg:'bg-[#E1F5EE]'},{v:'284',l:'Lots gérés',c:'text-blue-600',bg:'bg-blue-50'},{v:'3',l:'Sinistres',c:'text-amber-600',bg:'bg-amber-50'},{v:'1.4k€',l:'Impayés',c:'text-red-500',bg:'bg-red-50'}].map(({v,l,c,bg})=>(
  ...
  },
  {
    ...
    bullets: [
      ...
      'Vue carte ou liste selon vos préférences',
    ...
    preview: () => (
      <div className="space-y-2">
        ...
        ].map(({nom,ville,statut,sc})=>(
          <div key={nom} className="flex items-center justify-between bg-white border border-gray-100 rounded-xl p-3 hover:border-[#0F6E56]/30 transition-colors">
      ...
      <div className="space-y-3">
        <div className="bg-white rounded-xl border border-gray-100 overflow-hidden">
          ...
          {[
            ...
            <div key={n} className="grid grid-cols-3 px-3 py-2.5 border-t border-gray-50 text-xs hover:bg-gray-50 transition-colors">
  ...
  },
  {
    ...
    preview: () => (
      <div className="space-y-2.5">
        {[
          {ref:'ACH-2025-001',coproprietaire:'M. Martin',montant:'420 €',statut:'Payé',progress:100,pc:'bg-[#0F6E56]'},
          {ref:'ACH-2025-002',coproprietaire:'Mme Dupont',montant:'435 €',statut:'Partiel',progress:60,pc:'bg-amber-400'},
          {ref:'ACH-2025-003',coproprietaire:'M. Leroy',montant:'390 €',statut:'Impayé',progress:0,pc:'bg-red-400'},
        ].map(({ref,coproprietaire,montant,statut,progress,pc})=>(
          <div key={ref} className="bg-white border border-gray-100 rounded-xl p-3">
            <div className="flex items-center justify-between mb-2">
              <div>
                <div className="text-xs font-semibold text-[#1C1C1A]">{coproprietaire}</div>
                <div className="text-[10px] text-gray-400">{ref} · {montant}</div>
              </div>
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                statut==='Payé'?'bg-[#E1F5EE] text-[#0F6E56]':statut==='Partiel'?'bg-amber-50 text-amber-700':'bg-red-50 text-red-600'
              }`}>{statut}</span>
            </div>
            <div className="w-full bg-gray-100 rounded-full h-1.5">
              <div className={`${pc} h-1.5 rounded-full transition-all`} style={{width:`${progress}%`}} />
            </div>
          </div>
        ))}
      </div>
  ...
  },
  {
    ...
    preview: () => (
      <div className="space-y-2">
        {[
          {icon:'📋',cat:'Procès-verbaux AG',nb:'12 documents',date:'Dernier : 15 mai 2025'},
          {icon:'📄',cat:'Contrats & assurances',nb:'8 documents',date:'Dernier : 3 avr. 2025'},
          {icon:'🏗️',cat:'Plans & diagnostics',nb:'5 documents',date:'Dernier : 12 jan. 2025'},
          {icon:'📜',cat:'Règlements de copropriété',nb:'4 documents',date:'Dernier : 1 mars 2025'},
        ].map(({icon,cat,nb,date})=>(
          <div key={cat} className="flex items-center gap-3 bg-white border border-gray-100 rounded-xl p-3 hover:border-indigo-200 transition-colors group">
            <span className="text-xl">{icon}</span>
            <div className="flex-1 min-w-0">
              <div className="text-xs font-semibold text-[#1C1C1A] truncate">{cat}</div>
              <div className="text-[10px] text-gray-400">{nb} · {date}</div>
            </div>
  ...
  },
  {
    ...
    bullets: [
      ...
      'Suivi d\'avancement : signalé → expertise → travaux → clôturé',
    ...
    preview: () => (
      <div className="grid grid-cols-3 gap-2">
        {[
          {label:'Signalé',color:'border-amber-200 bg-amber-50',badge:'text-amber-700',items:['Dégât des eaux — Rés. Bellevue','Fissure façade — Le Clos']},
          {label:'En cours',color:'border-blue-200 bg-blue-50',badge:'text-blue-700',items:['Ascenseur en panne — Villa des Pins']},
          {label:'Clôturé',color:'border-[#0F6E56]/20 bg-[#E1F5EE]',badge:'text-[#0F6E56]',items:['Toiture — Les Hauts','Chaudière — Résid. Arc']},
        ].map(({label,color,badge,items})=>(
          <div key={label} className={`border rounded-xl p-2.5 ${color}`}>
            <div className={`text-[10px] font-bold mb-2 ${badge}`}>{label}</div>
  ...
  },
  {
    ...
    desc: 'Échangez directement avec vos copropriétaires depuis Coplio. Plus de boîtes email dispersées — toute l\'historique est centralisé et archivé.',
    bullets: [
      'Messagerie directe syndic ↔ copropriétaire',
    ...
    preview: () => (
      <div className="space-y-2.5">
        <div className="bg-white border border-gray-100 rounded-xl overflow-hidden">
          {[
            {n:'M. Martin',msg:'Bonjour, la fuite dans mon appartement n\'est toujours pas réparée...',time:'10:24',unread:true},
            {n:'Mme Dupont',msg:'Merci pour l\'envoi de la convocation AG.',time:'Hier',unread:false},
            {n:'M. Leroy',msg:'Pouvez-vous me renvoyer la quittance Q1 ?',time:'Lun.',unread:false},
          ].map(({n,msg,time,unread})=>(
            <div key={n} className={`flex items-start gap-3 px-3 py-2.5 border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors ${unread?'bg-[#F4F2EB]':''}`}>
              <div className="w-8 h-8 bg-[#0F6E56] rounded-full flex items-center justify-center flex-shrink-0">
                <span className="text-white text-[10px] font-bold">{n.split(' ')[1][0]}</span>
              </div>
              <div className="flex-1 min-w-0">
              ...
              {unread&&<div className="w-2 h-2 rounded-full bg-[#0F6E56] flex-shrink-0 mt-1" />}
            </div>
          ))}
        </div>
        <div className="flex gap-2">
          <input readOnly value="Bonjour M. Martin, notre technicien..." className="flex-1 text-[11px] bg-white border border-gray-100 rounded-xl px-3 py-2 text-gray-500" />
          <button className="bg-[#0F6E56] text-white px-3 py-2 rounded-xl">
